...
export const UserAuthModal: React.FC<UserAuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [phoneInput, setPhoneInput] = useState(() => {
    return localStorage.getItem('fast_miner_last_login_phone') || '';
  });
  ...
  const [rememberMe, setRememberMe] = useState(() => {
    return localStorage.getItem('fast_miner_remember_me') !== 'false';
  });

  const [lastLoginMethod, setLastLoginMethod] = useState<string | null>(() => {
    return localStorage.getItem('fast_miner_last_login_method');
  });
  const [lastGmail, setLastGmail] = useState<string | null>(() => {
    return localStorage.getItem('fast_miner_last_login_email');
  });
  ...
  const handleSocialLogin = async (provider: 'google' | 'apple') => {
    ...
    if (provider === 'google') {
      try {
        ...
        setErrorText('Google Single Sign-On was blocked by preview iframe sandboxing. Provisioning high-fidelity secure workspace fallback...');
        setTimeout(() => {
              ...
              <>
                ...
                <div className="relative">
                  {lastLoginMethod === 'google' && (
                    <span className="absolute -top-2 right-4 bg-emerald-500 text-slate-950 font-mono text-[8px] font-black uppercase px-2 py-0.5 rounded-full tracking-wider z-10 shadow-md animate-pulse">
                      Last Saved Method
                    </span>
                  )}
                  <button
                    onClick={() => handleSocialLogin('google')}
                    className={`w-full h-12 rounded-2xl border transition-all font-sans font-bold text-xs flex items-center justify-center gap-3 cursor-pointer group hover:shadow-lg ${
                      lastLoginMethod === 'google'
                        ? 'border-emerald-500 bg-emerald-500/10 hover:bg-emerald-500/15 text-white shadow-emerald-500/5'
                        : 'border-white/10 hover:border-[#4285F4]/50 bg-white/5 hover:bg-[#4285F4]/10 text-white/95 hover:shadow-[#4285F4]/5'
                    }`}
                  >
                    ...
                    <span>
                      {lastLoginMethod === 'google' && lastGmail
                        ? `Continue with ${lastGmail}`
                        : 'Sync Instantly with Google'}
                    </span>
                  </button>
                </div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`w-full h-12 rounded-2xl transition-all font-sans font-black text-xs flex items-center justify-center gap-3 cursor-pointer group shadow-lg ${
                      lastLoginMethod === 'apple'
                        ? 'bg-emerald-500 text-slate-950 hover:bg-emerald-400'
                        : 'bg-white text-slate-950 hover:bg-[#e4e4e4]'
                    }`}
                  >
                ...
                {!otpSent ? (
                  <form onSubmit={handlePhoneSubmit} className="space-y-3 font-mono relative">
                    {lastLoginMethod === 'phone' && (
                      <span className="absolute -top-2 right-4 bg-emerald-500 text-slate-950 font-mono text-[8px] font-black uppercase px-2 py-0.5 rounded-full tracking-wider z-10 shadow-md animate-pulse">
                        Last Saved Method
                      </span>
                    )}
                    <div className={lastLoginMethod === 'phone' ? 'border border-emerald-500/20 p-3 rounded-2xl bg-emerald-500/[0.02]' : ''}>
                ...
                {/* Google login issues diagnostic / tip card */}
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3.5 text-[10px] text-white/40 leading-relaxed font-mono">
                  <span className="block font-bold uppercase text-white/60 mb-1 flex items-center gap-1">
                    <Chrome className="h-3.5 w-3.5 text-[#4285F4]" />
                    GOOGLE AUTH DIAGNOSTICS & SANDBOX
                  </span>
                  <span>
                    Google Sign-In uses popups. If blocked by secure cross-origin iframe security policies, the station automatically transitions you into a high-fidelity cloud-synchronized sandbox. To experience authentic on-chain Google accounts syncing, consider opening the application in a new browser tab.
                  </span>
                </div>

                {/* Session Persistence Option Card */}
                <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3.5 flex items-center justify-between gap-3 font-mono mt-2 transition-all">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[8px] text-white/35 mt-1 leading-normal">
                      Retain active node authorization keys on this local device.
                    </span>
                  </div>
                  <button
                    type="button"
